...
new RtGrain[8] @=> Master.rtGrain;


for (0=>int i;i<8;i++)
{
	Master.chan[i] =>  Master.pan[i] =>Master.volume;
	Master.chan[i]=>Master.rtGrain[i]=>Master.pan[i];
	spork ~Master.rtGrain[i].run();
}

Master.volume => Master.rev =>dac;// => Master.comp =>  Master.limit=> dac;
//Master.volume => dac;
//Master volume
